...
function findphonexy(res, e) {
      ...
      if(yDown  < 01 * 0.0182*h ){num = 01}
                ...
                if(downed){ 
                  ...
                  var original = 120/xabs*8
                  if(num<9){
                    $('.whole_back #box0'+num).css({'width' : 0.8*w})
                  }else{
                    $('.whole_back #box'+num).css({'width' : 0.8*w})
                  }
                  var result = 120/xabs*8
                  // if(result< w){
                    console.log(w - 50/xabs)
                    console.log(50/xabs)
                      {divs[i].style.width = w - 1000/xabs + "px"}
                      {divs_back[i].style.width = (1000/xabs) + "px"}
                  // }else{
                    // divs[i].style.width = 0.1 + "px"
                    // divs_back[i].style.width = (w - 0.1) + "px"
                  // }
                }
               }
        ...
        if (res == 'up' || res == "out") {
            ...
            $(".whole .boxes").animate({width: w + "px"},{ duration: 500, queue: false });
            $(".boxes").animate({marginTop: 0 + "px"},{ duration: 500, queue: false });
            $(".whole_back .boxes").animate({width: w + "px"},{ duration: 500, queue: false });
            downed = false
          }
}               
...
function findxy(res, e) {
      ...
      if(yDown  < 01 * 0.0182*h ){num = 01}
                ...
                if(downed){ 
                  xabs = Math.abs(num-divnumbers)+1;
                  var result = (((w) - (w)/xabs)+1)*2
                  if(result> 0){
                      {divs[i].style.width =  w - 30/xabs+ "px"}
                      {divs_back[i].style.width = ( w - 30/xabs ) + "px"}
                  }else{
                    divs[i].style.width = 0.1 + "px"
